...
module.exports = function (screen, windows, sbot, filter) {
  ...
  feedsList.on('select', function (el, selected) {
    if (!feedsList.data.feeds[selected])
      return
    var feedviewslist = require('./feedviewslist')(screen, windows, sbot, feedsList.data.feeds[selected].id)
    windows.push(feedviewslist)
  })
